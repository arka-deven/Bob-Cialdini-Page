...
import {
  emailSignupWithPhoneSchema,
  phoneSchema,
  otpSchema,
  type EmailSignupWithPhoneInput,
  type PhoneInput,
  type OtpInput,
...
export default function SignupPage() {
  ...
  const emailForm = useForm<EmailSignupWithPhoneInput>({ resolver: zodResolver(emailSignupWithPhoneSchema) });
  ...
  async function onEmailSubmit(data: EmailSignupWithPhoneInput) {
    setLoading(true);
          ...
          <CardContent className="space-y-6">
            ...
            {mode === "email" ? (
              <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
                ...
                <div>
                  <Input type="tel" placeholder="Phone number (+1 555 000 0000)" {...emailForm.register("phone")} />
                  {emailForm.formState.errors.phone && <p className="mt-1 text-xs text-destructive">{emailForm.formState.errors.phone.message}</p>}
                </div>
